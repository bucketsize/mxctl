...
local function geturlonsuccess(url)
	local headers, stream, err = http_request.new_from_uri(url):go()
	if err then
...
function F:getwallpaper(provider)
	...
	local stream1, err1 = assert(geturlonsuccess(pro.url()))
	if err1 then
	...
	local stream, err = assert(geturlonsuccess(url))
	if err then
		logger.info("Error getting url %s, %s", url, err)
	end
	local file = assert(io.open(wlprs .. name, "w"))
...
local function exec_select(eopts, fn)
	local labels = {}
	...
	Pr.pipe()
		.add(Sh.exec(menu_sel(string.format('echo "%s"', table.concat(labels, "\n")))))
		.add(function(k)
			if k then
